...
\author{\\
\name Alfonso Tejeda Rodríguez \email \href{mailto:alfonsotejeda150605@gmail.com}{alfonsotejeda150605@gmail.com}
	\\ \addr Grupo C212 \AND
...
\begin{tabular}{cccc}
\toprule
\textbf{Tiempo (s)} & \textbf{Euler} & \textbf{RK2} & \textbf{RK4} \\
\midrule
5 & 0.625 & 0.031 & $<0.001$ \\
...
\begin{tabular}{cccc}
\toprule
\textbf{Paso $h$} & \textbf{Euler} & \textbf{RK2} & \textbf{RK4} \\
\midrule
...
La ecuación $\frac{dv}{dt} = rv - v^3$ describe sistemas que presentan transiciones cualitativas en su comportamiento dinámico cuando varía el parámetro $r$.
...
\begin{tabular}{ccc}
\toprule
\textbf{Región} & \textbf{$f'(0)$} & \textbf{Estabilidad} \\
\midrule
$r<0$ & $r<0$ & Estable \\
$r=0$ & $0$ & No concluyente\\
$r>0$ & $r>0$ & Inestable \\
\bottomrule
\end{tabular}
\caption{Estabilidad de $v=0$ según el parámetro $r$}
\label{tab:estab_v0}
\end{table}
...
\begin{itemize}
  \item \textbf{Discriminante:} $\Delta = \alpha^2 - 4\beta$ controla la naturaleza de la convergencia y determina el tipo de amortiguamiento del sistema, pudiendo tomar cualquier valor real.
...
El comportamiento dinámico depende del \textbf{discriminante} ${\Delta = \alpha^2 - 4\beta}$. A continuación se presentan los tres casos posibles:
...
\begin{tabular}{lccc}
\toprule
\textbf{Propiedad} & \textbf{Sobre-amort.} & \textbf{Crítico} & \textbf{Sub-amort.} \\
\midrule
Condición & $\Delta > 0$ & $\Delta = 0$ & $\Delta < 0$ \\
Valores propios & Reales & Repetido & Complejos \\
Tipo geométrico & Nodo & Nodo crítico & Foco \\
Oscilaciones & No & No & Sí \\
Vel. retorno & Lenta & Óptima & Media \\
\bottomrule
\end{tabular}
...
El análisis completo del sistema masa-resorte-amortiguador demuestra que para $\alpha, \beta > 0$, el sistema es \textbf{globalmente asintóticamente estable}. Independientemente de las condiciones iniciales, el sistema siempre converge al punto de equilibrio $(0, 0)$, y la energía se disipa completamente. La visualización en el plano de fase confirma que todos los casos de amortiguamiento resultan en trayectorias convergentes al equilibrio.
